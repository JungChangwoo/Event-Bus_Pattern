...
public class ClientInputMain {
		...
		while (!done) {
			...
			try {
				...
				case "4":
					...
					break;
				case "5":
					eventBus.sendEvent(new Event(EventId.DeleteStudents, setId("student")));
					printLogSend(EventId.DeleteStudents);
					break;
				case "6":
					eventBus.sendEvent(new Event(EventId.DeleteCourses, setId("course")));
					printLogSend(EventId.DeleteStudents);
					break;
				case "7":
					...
					break;
				case "8":
					eventBus.sendEvent(new Event(EventId.MakeReservations, setId("student", "course")));
					printLogSend(EventId.MakeReservations);
					break;
				case "0":
	...
	private static String setId(String object) throws IOException {
		String info = "\nEnter " + object + " ID and press return ";
		if(object.equals("student") || object.equals("Student")) {
			info += "(Ex. 20131234)>> ";
		} else if(object.equals("course") || object.equals("Course")) {
			info += "(Ex. 12345)>> ";
		}
		System.out.println(info);
		return new BufferedReader(new InputStreamReader(System.in)).readLine().trim();
	}
	private static String setId(String object1, String object2) throws IOException {
		String userInput = "";
		userInput += setId(object1);
		userInput += " " + setId(object2);
		return userInput;
	}
